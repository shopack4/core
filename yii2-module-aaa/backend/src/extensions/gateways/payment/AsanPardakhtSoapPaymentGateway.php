	...
	implements IPaymentGateway
{
	...
	public function prepare(&$gatewayModel, $onlinePaymentModel, $callbackUrl)
	{
		...
		$req = "1,{$username},{$password},{$orderId},{$price},{$localDate},{$additionalData},{$callBackUrl},0";
			//اگر قصد واریز پول به چند شبا را دارید، خط زیر را به رشته بالایی اضافه کنید
			// ,Shaba1,Mablagh1,Shaba2,Mablagh2,Shaba3,Mablagh3
			//حداکثر تا 7 شبا می‌توانید به رشته خود اضافه کنید
	...
	public function verify(&$gatewayModel, $onlinePaymentModel, $pgwResponse)
	{
		...
		try {
			...
			// echo '<div class="error-bank">خطا در فراخوانی وب‌سرویس.</div>';
			// exit();
		}
		...
		// or die("خطای فراخوانی متد وریفای.");
	...
	function EncryptWS($string = "")
	{
		...
		try {
			...
			// echo "<div class=\"error\">خطا در فراخوانی وب‌سرویس رمزنگاری.</div>";
			// exit();
	...
	public function paymentinvoice()
	{
		if (isset($id)) {
			...
			try {
					...
					// echo "<div class=\"error\">{$E->faultstring}</div>";
					echo "<div class=\"error\">خطا در فراخوانی وب‌سرویس.</div>";
					exit();
			}
			$params = array(
...
public function successinvoice()
{
	...
	//echo('<div class="success-bank">پرداخت با موفقیت انجام پذیرفت.</div>');
	echo ('<div class="success-bank">
پرداخت با موفقیت انجام پذیرفت.
<br>
کدپیگیری : ' . $RRN . '</div>');
