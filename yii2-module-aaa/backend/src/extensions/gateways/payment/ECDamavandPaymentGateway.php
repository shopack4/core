	...
	implements IPaymentGateway
{
	...
	const URL_PayReverse        = "https://ecd.shaparak.ir/ipg_ecd/PayReverse";
	...
	public function verify(&$gatewayModel, $onlinePaymentModel, $pgwResponse)
	{
		try
		{
			...
			*/
			...
			$fnRejectAndRaiseError = function($err) use ($res_Token) {
				try {
					$this->reverseRequest($res_Token);
				} catch (\Throwable $exp) {
					$err .= " - " . $exp->getMessage();
					//throw $th;
				}
			...
			if ($res_State != 1) {
				$err = $this->formatErrorInfo($res_ErrorCode, $res_ErrorDescription);
				$fnRejectAndRaiseError($err);
			}

			$price = $onlinePaymentModel->onpAmount * 10; //toman -> rial
			if ($res_Amount != $price)
				$fnRejectAndRaiseError('Invalid amounts');

			if ($res_BuyID != $onlinePaymentModel->onpID)
				$fnRejectAndRaiseError('Invalid payment id');
			...
			$curl = curl_init();
			curl_setopt_array($curl, [
				CURLOPT_URL							=> self::URL_PayConfirmation,
				CURLOPT_RETURNTRANSFER	=> 1,
				CURLOPT_POST						=> 1,
				CURLOPT_POSTFIELDS			=> $data_string,
				CURLOPT_HTTPHEADER			=> [
	...
	protected function reverseRequest($token)
	{
		$params = [
			'Token'	=> $token,
		];
		...
		$curl = curl_init();
		curl_setopt_array($curl, [
			CURLOPT_URL							=> self::URL_PayReverse,
			CURLOPT_RETURNTRANSFER	=> 1,
			CURLOPT_POST						=> 1,
			CURLOPT_POSTFIELDS			=> $data_string,
			CURLOPT_HTTPHEADER			=> [
