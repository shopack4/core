...
class Authenticator
{
	...
	public static function generateRandomSecret($secretLength = 16)
	{
		$secret = '';
		$validChars = array(
			'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H',
		...
		if ($secretLength < 16 || $secretLength > 128) {
			throw new UnprocessableEntityHttpException('Bad secret length');
		}
		$random = false;
		...
		if ($random !== false) {
			...
			throw new UnprocessableEntityHttpException('Cannot create secure random secret due to source unavailbility');
		}
	...
	public static function getCode($secret, $timeSlice = null)
	{
		...
		$secretkey = self::debase32($secret);
		...
		$modulo = pow(10, self::$length);

		return str_pad($value % $modulo, self::$length, '0', STR_PAD_LEFT);
	}

	public static function getQR($name, $secret, $title = null, $params = array())
	{
	...
	protected static function debase32($secret)
	{
		if (empty($secret)) {
			return '';
		}
