...
class m240901_144811_aaa_add_unique_transaction_number_to_online_payment extends Migration
{
	public function safeUp()
	{
		$this->execute(<<<SQL
ALTER TABLE `tbl_AAA_OnlinePayment`
	ADD COLUMN `onpTransactionNumber` VARCHAR(64) NULL DEFAULT NULL AFTER `onpPaymentToken`;
SQL
		);
...
CREATE TRIGGER trg_updatelog_tbl_AAA_OnlinePayment AFTER UPDATE ON tbl_AAA_OnlinePayment FOR EACH ROW BEGIN
  ...
  IF ISNULL(OLD.onpTransactionNumber) != ISNULL(NEW.onpTransactionNumber) OR OLD.onpTransactionNumber != NEW.onpTransactionNumber THEN SET Changes = JSON_MERGE_PRESERVE(Changes, JSON_OBJECT("onpTransactionNumber", IF(ISNULL(OLD.onpTransactionNumber), NULL, OLD.onpTransactionNumber))); END IF;
...
SQL
	...
	{
		echo "m240901_144811_aaa_add_unique_transaction_number_to_online_payment cannot be reverted.\n";
		return false;
	}
