  ...
  INNER JOIN (
      SELECT usrID
           , JSON_REMOVE(JSON_OBJECT(
             ...
             , CASE WHEN usrSSID        IS NULL THEN '$.ssid'        ELSE '$.dummy' END
             , CASE WHEN usrBirthCertID IS NULL THEN '$.birthCertID' ELSE '$.dummy' END
             , CASE WHEN usrBirthDate   IS NULL THEN '$.birthDate'   ELSE '$.dummy' END
             ) AS _2fa
        FROM tbl_AAA_User
             ) tmp
          ON tmp.usrID = tbl_AAA_User.usrID
         SET usr2FA = IF(JSON_LENGTH(_2fa) = 0, NULL, _2fa)
       WHERE usr2FA IS NULL
;
SQL
    );
...
CREATE TRIGGER trg_updatelog_tbl_AAA_User AFTER UPDATE ON tbl_AAA_User FOR EACH ROW BEGIN
  ...
  IF ISNULL(OLD.usr2FA) != ISNULL(NEW.usr2FA) OR OLD.usr2FA != NEW.usr2FA THEN SET Changes = JSON_MERGE_PRESERVE(Changes, JSON_OBJECT("usr2FA", IF(ISNULL(OLD.usr2FA), NULL, OLD.usr2FA))); END IF;
