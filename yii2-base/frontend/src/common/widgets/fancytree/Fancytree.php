...
namespace shopack\base\frontend\common\widgets\fancytree;
...
EOJS
	...
	public function init(): void
	{
		...
		if (!empty($this->options['skin'])) {
			FancytreeAsset::$skin = $this->options['skin'];
		}
		if (
...
function(event, data) {
	$.ui.fancytree.getTree({$selector}).activateKey('{$activeNode}');
}
EOJS
			);
