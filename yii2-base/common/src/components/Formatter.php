...
// if (is_array($value)) die(Html::dump($value));
	...
	public function asPhone($value, $nullValue = null)
	{
		...
		$value = str_replace(' ', '-', PhoneHelper::formatPhoneNumber($value, PhoneNumberFormat::INTERNATIONAL));

		return "<span class='d-inline-block text-nowrap dir-ltr text-start'>"
			. $value
			. "</span>";

		// return "<div class='text-nowrap dir-ltr text-start'>"
		// 	. $value
		// 	. "</div>";
	...
	// }
	public function asToman($value, $nullLabel = null)
	{
		if (empty($value))
			return 0;

		return $this->asDecimal($value) . ' تومان';
	}
